 # Batched 1D FFT Benchmark                                                                                                                                                            
  ...
  - **N ≤ 2048 (shared-memory kernel)** — one CUDA block per FFT, all N elements loaded into shared memory. The entire computation happens in shared memory with no global memory
  traffic, then results are written back once. Launched with N/2 threads per block.                                                                                                     
                  
  - **N > 2048 (global-memory multi-pass)** — shared memory is too small, so data stays in global memory. A separate `bit_reverse_kernel` permutes the input, then `fft_butterfly_stage`
   is called once per DIT stage (log2(N) launches total).
